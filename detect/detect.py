...
def detect_licese(file, norm, enhance=True, kill_curve=False, show_img=True, enhance_speed=2, vertical_coe = 12, iter_coe=4, hw_rate=(1.8, 8.8),
    ...
    img5 = cv2.erode(img4, None, iterations = iter_coe)
    img5 = cv2.dilate(img5, None, iterations = iter_coe)
    ...
    def hwRateFilter(x):
        return x[2] > x[3] * hw_rate[0] and x[2] < x[3] * hw_rate[1]
    def vertialFilter(x):
